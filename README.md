...
Run `sudo apt update -y && sudo apt install python3-pip python3.9 python3 git screen libopus-dev nano libffi-dev libsodium-dev build-essential -y && git clone https://github.com/rexjohannes/volumio-bot.git && cd volumio-bot && python3.9 -m pip install --upgrade -r requirements.txt && chmod +x run.sh && nano ./config/token.txt` in your Putty. Enter `volumio` as password.
