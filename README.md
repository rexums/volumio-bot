...
Run `sudo apt update -y && sudo apt install python3-pip python3 git screen libopus-dev -y && git clone https://github.com/rexjohannes/volumio-bot.git && cd volumio-bot && python3 -m pip install --upgrade -r requirements.txt && chmod +x run.sh && nano ./config/token.txt` in your Putty. Enter `volumio` as password.
